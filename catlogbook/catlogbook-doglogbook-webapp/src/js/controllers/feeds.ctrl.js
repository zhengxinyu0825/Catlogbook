(function(){
  ...
  function FeedsCtrl(
    ...
    function getResults(id, datePeriod){
      ...
      PetService.getFeed(vm.selectedPet.id,getOptions).then(
        (results) => {
          vm.results = results;
    ...
    function getFoodItems(items){
      return items.map((i)=> { return i.foodType.name;}).join(', ');
    }
    ...
    function remove(id){
      if (id){
        ngDialog.openConfirm({
            ...
            PetService.removeFeed($rootScope.selectedPet.id, id).then(
              (result) => {
